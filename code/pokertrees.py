...
class GameTree(object):
    def __init__(self, rules, teams=None):
    ...
    def showdown(self, root, players_in, committed, holes, board, deck, bet_history):
        ...
        for w in winners:
            payoffs[w] += payoff
        # share the payoff among team members
        if self.teams is not None:
            for team in self.teams:
                team_payoff = 0
                for player in team:
                    team_payoff += payoffs[player]
                for player in team:
                    payoffs[player] = team_payoff / float(len(team))
...
class PublicTree(GameTree):
    def __init__(self, rules, teams=None):
        GameTree.__init__(self, GameRules(rules.players, rules.deck, rules.roundinfo, rules.ante, rules.blinds, rules.handeval, partial(multi_infoset_format, rules.infoset_format)), teams)
        ...
        if players_in.count(True) == 1:
            ...
            if self.teams is not None:
                for team in self.teams:
                    team_payoff = 0
                    for player in team:
                        team_payoff += fold_payoffs[player]
                    for player in team:
                        fold_payoffs[player] = team_payoff / float(len(team))
